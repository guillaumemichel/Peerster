...
do
	...
	nohup tilix -t $name -s $name --window-style=disable-csd-hide-toolbar -e bash -c './Peerster -name '$name' -GUIPort '$UIport' -UIPort '$UIport' -gossipAddr 127.0.0.1:'$Gport' -rtimer 10 -peers 127.0.0.1:5000; exec bash' > /dev/null 2>&1 &
	((counter++))
done
